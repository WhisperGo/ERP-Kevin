...
class M_siswa extends Model
{
    ...
    {
        // Ambil jurusan dari tabel rombel
        $jurusan = $this->db->table('siswa')
            ->select('rombel.jurusan') // Pilih kolom jurusan dari tabel rombel
            ->join('rombel', 'rombel.id_rombel = siswa.rombel') // Bergabung dengan tabel rombel berdasarkan kolom rombel pada tabel guru dan id_rombel pada tabel rombel
            ->get()
            ->getRow();

        // Gunakan nilai jurusan sebagai kriteria dalam pengambilan data dari tabel siswa
        return $this->db->table($table1)
            ->join($table2, $on, 'left')
            ->join($table3, $on2, 'left')
            ->select(
                $table1 . '.*,'
                    . $table1 . '.user as user_siswa,'
                    . $table2 . '.*,'
                    . $table3 . '.*,'
            )
            ->where('siswa.jurusan', $jurusan ? $jurusan->jurusan : null) // Gunakan jurusan dari tabel rombel sebagai kriteria jika ditemukan
